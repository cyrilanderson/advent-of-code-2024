...
def perform_swap(gates, swap):
    ...
    out1 = swap[0]
    out2 = swap[1]
    gate1 = next(gate for gate in gates if gate[3] == out1)
    gate2 = next(gate for gate in gates if gate[3] == out2)
    gate1_new = (gate1[0], gate1[1], gate1[2], out2)
    gate2_new = (gate2[0], gate2[1], gate2[2], out1)
    new_gates.remove(gate1)
    new_gates.remove(gate2)
    new_gates.append(gate1_new)
    new_gates.append(gate2_new)
    return new_gates
...
def main():
    ...
    comp_graph, valid = create_computational_graph(inits, gates)
    completed_graph = run_computational_graph(comp_graph)
    z_str, _ = get_z_result(completed_graph)
    ...
    x = int(x_inits_string, 2)
    y = int(y_inits_string, 2)
    sum = x + y
    bin_sum = bin(sum)[2:]
    difference = edit_distance(z_str, bin_sum)
    #target_output = int(x_inits_string, 2) + int(y_inits_string, 2)

    good_swaps = set()

    swaps_to_check = [(gate1[3], gate2[3]) for gate1 in gates for gate2 in gates if gate1[3] != gate2[3]]
    #print(swaps_to_check)
    for idx, swap in enumerate(swaps_to_check):
        print(f"Checking swap: {swap}, {idx} of {len(swaps_to_check)}")
        new_gates = perform_swap(gates, swap)
        comp_graph_test, valid = create_computational_graph(inits, new_gates)
        if valid:
            completed_test_graph = run_computational_graph(comp_graph_test)
            z_test, _ = get_z_result(completed_test_graph)
            diff = edit_distance(z_test, bin_sum)
            if diff < difference:
                good_swaps.add(swap)
                print(f"Good swap: {swap}, diff: {diff}")
    
    print(f"Good swaps: {good_swaps}")
