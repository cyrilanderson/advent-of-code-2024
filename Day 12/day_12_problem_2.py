...
def count_corners(connected_region):
    corners = 0
    region = connected_region["squares"]
    for square in region:
        row, col = square
        for row_offset, col_offset in [(-1, -1), (-1, 1), (1, -1), (1, 1)]:
            diag_neigh_row = row + row_offset
            diag_neigh_col = col + col_offset
            convex = (diag_neigh_row, diag_neigh_col) not in region and (diag_neigh_row, col) in region and (row, diag_neigh_col) in region
            #concave = (diag_neigh_row, diag_neigh_col) not in region and (diag_neigh_row, col) not in region and (row, diag_neigh_col) not in region
            #kisses_self_corner = (diag_neigh_row, diag_neigh_col) in region and (diag_neigh_row, col) not in region and (row, diag_neigh_col) not in region
            concave = (diag_neigh_row, col) not in region and (row, diag_neigh_col) not in region
            #if convex or concave or kisses_self_corner:
...
def main():
    ...
    connected_regions = compute_connected_regions(map)
    total_price = 0
    area_perim_price = 0
    for connected_region in connected_regions:
        sides = count_corners(connected_region)
        total_price += connected_region["area"] * sides
        area_perim_price += connected_region["area"] * connected_region["perimeter"]
    
    # for region in areas:
    #     total_price += areas[region] * perimeters[region]
    print(f"Total sides-area price: {total_price}")
    print(f"Total area-perimeter price: {area_perim_price}")
    # print(areas)
    # print(perimeters)
    # print(len(areas))
    end_time = time.time()
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        print("Usage: python day_12_problem_2.py <file_path>")
    else:
        main()